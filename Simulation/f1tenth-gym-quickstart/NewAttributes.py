...
class NewStuff:
    ...
    def process_lidar(self, scan, x, y):
        ...
        front_range = np.concatenate((scan[:10], scan[-10:]))
        min_front_dist = np.min(front_range)
        print(f"Min distance in front: {min_front_dist:.2f} | Speed: {speed:.2f} | Location: ({x:.2f}, {y:.2f})")
